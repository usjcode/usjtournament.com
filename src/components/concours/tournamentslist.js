...
function TournamentUpdateDialog(props) {
  const [value, setValue] = React.useState(moment().add(2, "months"));
  const [dated, setdated] = React.useState(moment().add(4, "months"));
    ...
    const response = await axios.patch(`http://127.0.0.1:8000/tournaments/${update}`,
      {
        ...
        "date_inscription": value.format("YYYY-MM-DD"),
...
function TournamentAddDialog(props) {
  ...
  const [datei, setdatei] = React.useState(moment().add(2, "months"));
  const [dated, setdated] = React.useState(moment().add(4, "months"));
